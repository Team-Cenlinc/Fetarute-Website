...
export function setupHomeGallery() {
  ...
  if (homeGallery && homeGalleryStage && homeGalleryTrack && homeGalleryItems.length > 0) {
    ...
    const getHomeGalleryDocumentTop = () =>
      homeGallery.getBoundingClientRect().top + window.scrollY;
    ...
    const writeHomeGalleryFrame = (measurement: HomeGalleryFrameMeasurement) => {
      ...
      if (
        document.documentElement.hasAttribute("data-home-gallery-pinned") !== measurement.pinned
      ) {
