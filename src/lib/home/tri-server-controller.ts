...
let disposeHomeTriServer: (() => void) | undefined;

/**
 * 在三服章节进入预加载视口后启用车窗、轮播与按需地图。
 * 清理器集中停止所有异步工作，确保离开文档后不再写入旧节点。
 */
export function setupHomeTriServer(): () => void {
  if (disposeHomeTriServer) return disposeHomeTriServer;

  const homeTriServer = document.querySelector<HTMLElement>("[data-home-tri-server]");
  if (!homeTriServer) return () => {};
  ...
  const mapLoadingTimers = new Set<number>();
  ...
  const setMapCardState = (panel: HTMLElement, isOpen: boolean) => {
    ...
    const launchIcon = launchButton?.querySelector<HTMLElement>('[data-map-button-icon="launch"]');
  ...
  const readActiveServerFromScroll = () => {
    ...
    const activeStop =
      ...
      measuredStops.reduce((closestStop, candidate) => {
        const distance = Math.abs(candidate.bounds.top + candidate.bounds.height / 2 - readingLine);
  ...
  mediaPanels.forEach((panel) => {
    ...
    const prepareSlideIndex = async (
      ...
      if (isDisposed || requestId !== imageLoadRequest) {
        return false;
      }
    ...
    function resumeAutoplayCycle() {
      ...
      autoplayTimer = window.setTimeout(
        async () => {
          ...
          const currentIndex = Number(panel.dataset.activeSlide ?? 0);
          const nextIndex = getHomeTriServerCarouselAutoplayNextIndex(currentIndex, slides.length);
          ...
          // 图片 decode 可以跨越 pagehide；清理后不能重置旧面板的进度或重新登记轮播周期。
    ...
    window.addEventListener("pointerup", endCarouselPointerInteraction, { signal });
    window.addEventListener("pointercancel", endCarouselPointerInteraction, { signal });
    carousel.addEventListener("wheel", () => resetAutoplayCycle(true), { passive: true, signal });
    carousel.addEventListener("scroll", updateSlideIndexFromScroll, { passive: true, signal });
    carousel.addEventListener(
      "keydown",
      (event) => {
    ...
    panel.addEventListener(
      "focusout",
      () => {
        focusWithinFrame = window.requestAnimationFrame(() => {
    ...
    document.addEventListener("visibilitychange", syncAutoplayState, { signal });
    reducedTriServerMotionQuery.addEventListener("change", () => resetAutoplayCycle(), { signal });
    desktopTriServerQuery.addEventListener("change", refreshPanelObservation, { signal });
    mediaPanelObserverCleanup.add(() => {
      window.cancelAnimationFrame(pendingCarouselFrame);
      window.cancelAnimationFrame(focusWithinFrame);
      window.clearTimeout(carouselScrollSettleTimer);
      window.clearTimeout(imageLoadResetTimer);
      window.clearTimeout(autoplayTimer);
      cancelPendingSlideRequest();
      ...
      async () => {
        ...
        iframe.addEventListener(
          ...
          () => {
            ...
            const mapLoadingTimer = window.setTimeout(() => {
              mapLoadingTimers.delete(mapLoadingTimer);
              if (isDisposed || !iframe.isConnected) {
                return;
              }
            ...
            mapLoadingTimers.add(mapLoadingTimer);
          },
          { once: true, signal },
        );
  ...
  desktopTriServerQuery.addEventListener("change", syncMediaPlacement, { signal });
  ...
  const dispose = (): void => {
    ...
    mediaPanels.forEach((panel) => closeInteractiveMap(panel));
    window.clearTimeout(serverLoadingResetTimer);
    mapLoadingTimers.forEach((timer) => window.clearTimeout(timer));
    mapLoadingTimers.clear();
    ...
    disposeHomeTriServer = undefined;
  };

  syncMediaPlacement();
  disposeHomeTriServer = dispose;
  return dispose;
}
