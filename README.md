...
当前三语首页已实现启动导视、出发验票、随机实景 Arrival、服联快线章节、响应式铁路图片 Gallery 与列车快选。Gallery 在桌面将纵向阅读进度映射为横向浏览，在小屏改为带当前焦点变化的原生纵向图片列。公告与指南内容仍保留在 Content
Collections 中，但首页暂不展示公告或加入入口；真实信息确认后再恢复对应界面。
